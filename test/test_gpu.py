...
def check_metrics(Z, base, prefix):
    ...
    print(
        "For {name}, mean r = {cor:.4f}, mean L2 error = {err:.4f}.".format(
            name=prefix, cor=np.mean(cors), err=np.mean(errors)
        )
    )
...
def plot_umap(adata, Z_cpu, Z_gpu, Z_R, prefix, batch_key):
    ...
    if os.system(
        "pegasus plot scatter --basis umap --attributes {attr} --alpha 0.5 ./result/{prefix}_result.h5ad ./plots/{prefix}.before.umap.pdf".format(
            attr=batch_key, prefix=prefix
        )
    ):
        sys.exit(1)

    if os.system(
        "pegasus plot scatter --basis umap_cpu --attributes {attr} --alpha 0.5 ./result/{prefix}_result.h5ad ./plots/{prefix}.cpu.umap.pdf".format(
            attr=batch_key, prefix=prefix
        )
    ):
        sys.exit(1)
    ...
    if os.system(
        "pegasus plot scatter --basis umap_gpu --attributes {attr} --alpha 0.5 ./result/{prefix}_result.h5ad ./plots/{prefix}.gpu.umap.pdf".format(
            attr=batch_key, prefix=prefix
        )
    ):
        sys.exit(1)
    ...
    if os.system(
        "pegasus plot scatter --basis umap_harmony --attributes {attr} --alpha 0.5 ./result/{prefix}_result.h5ad ./plots/{prefix}.harmony.umap.pdf".format(
            attr=batch_key, prefix=prefix
        )
    ):
        sys.exit(1)
