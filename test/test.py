...
def check_metric(Z_torch, Z_py, Z_R, prefix, norm):
    ...
    print(
        "Mean {metric} by harmony-pytorch = {value:.4f}".format(
            metric=metric_dict[norm], value=np.mean(metric_torch)
        )
    )
    np.savetxt(
        "./result/{prefix}_{metric}_torch.txt".format(prefix=prefix, metric=norm),
        metric_torch,
    )
    ...
    print(
        "Mean {metric} by harmonypy = {value:.4f}".format(
            metric=metric_dict[norm], value=np.mean(metric_py)
        )
    )
    np.savetxt(
        "./result/{prefix}_{metric}_py.txt".format(prefix=prefix, metric=norm),
        metric_py,
    )
...
def plot_umap(adata, Z_torch, Z_py, Z_R, prefix, batch_key):
    ...
    if os.system(
        "pegasus plot scatter --basis umap --attributes {attr} --alpha 0.5 ./result/{name}_result.h5ad ./plots/{name}.before.umap.pdf".format(
            name=prefix, attr=batch_key
        )
    ):
        sys.exit(1)

    if os.system(
        "pegasus plot scatter --basis umap_torch --attributes {attr} --alpha 0.5 ./result/{name}_result.h5ad ./plots/{name}.torch.umap.pdf".format(
            name=prefix, attr=batch_key
        )
    ):
        sys.exit(1)
    ...
    if os.system(
        "pegasus plot scatter --basis umap_py --attributes {attr} --alpha 0.5 ./result/{name}_result.h5ad ./plots/{name}.py.umap.pdf".format(
            name=prefix, attr=batch_key
        )
    ):
        sys.exit(1)
    ...
    if os.system(
        "pegasus plot scatter --basis umap_harmony --attributes {attr} --alpha 0.5 ./result/{name}_result.h5ad ./plots/{name}.harmony.umap.pdf".format(
            name=prefix, attr=batch_key
        )
    ):
        sys.exit(1)
...
def test_cell_lines():
    ...
    z_files = [
        f for f in os.listdir("./result") if re.match("cell_lines.*_z.(txt|npy)", f)
    ]
    ...
    if os.path.exists("./result/cell_lines_torch_z.npy"):
        ...
        print(
            "Time spent for harmony-pytorch = {:.2f}s.".format(end_torch - start_torch)
        )
    ...
    if os.path.exists("./result/cell_lines_result.h5ad"):
        ...
        print(
            "kBET for Harmony: statistic = {stat}, p-value = {pval}, ac rate = {ac_rate}".format(
                stat=stat, pval=pvalue, ac_rate=ac_rate
            )
        )

        stat, pvalue, ac_rate = pg.calc_kBET(adata, attr="dataset", rep="py")
        print(
            "kBET for harmonypy: statistic = {stat}, p-value = {pval}, ac rate = {ac_rate}".format(
                stat=stat, pval=pvalue, ac_rate=ac_rate
            )
        )
        ...
        print(
            "kBET for harmony-pytorch: statistic = {stat}, p-value = {pval}, ac rate = {ac_rate}".format(
                stat=stat, pval=pvalue, ac_rate=ac_rate
            )
        )
...
def test_mantonbm():
    ...
    z_files = [
        f for f in os.listdir("./result") if re.match("MantonBM.*_z.(txt|npy)", f)
    ]
    if len(z_files) < 3 or not os.path.exists("./result/MantonBM_result.h5ad"):
        adata = pg.read_input("./data/MantonBM/original_data.h5ad")
        adata.obs["Individual"] = pd.Categorical(
            adata.obs["Channel"].apply(lambda s: s.split("_")[0][-1])
        )
...
def gen_plot(norm):
    ...
    df1 = pd.DataFrame(
        {
            "dataset": np.repeat(
                ["Cell Lines"], metric_celllines_torch.size + metric_celllines_py.size
            ),
            "package": np.concatenate(
                (
                    np.repeat(["Torch"], metric_celllines_torch.size),
                    np.repeat(["Py"], metric_celllines_py.size),
                ),
                axis=0,
            ),
            "metric": np.concatenate(
                (metric_celllines_torch, metric_celllines_py), axis=0
            ),
        }
    )
    ...
    df2 = pd.DataFrame(
        {
            "dataset": np.repeat(
                ["10x PBMC"], metric_pbmc_torch.size + metric_pbmc_py.size
            ),
            "package": np.concatenate(
                (
                    np.repeat(["Torch"], metric_pbmc_torch.size),
                    np.repeat(["Py"], metric_pbmc_py.size),
                ),
                axis=0,
            ),
    ...
    df3 = pd.DataFrame(
        {
            "dataset": np.repeat(
                ["Bone Marrow"], metric_mantonbm_torch.size + metric_mantonbm_py.size
            ),
            "package": np.concatenate(
                (
                    np.repeat(["Torch"], metric_mantonbm_torch.size),
                    np.repeat(["Py"], metric_mantonbm_py.size),
                ),
                axis=0,
            ),
            "metric": np.concatenate(
                (metric_mantonbm_torch, metric_mantonbm_py), axis=0
            ),
        }
    )
    ...
    ax.set_title(
        "{} between Harmonypy and Harmony-pytorch Integration".format(metric_dict[norm])
    )
